...
@property (assign, nonatomic, getter = isExecuting) BOOL myexecuting;
@property (assign, nonatomic, getter = isFinished) BOOL myfinished;
...
- (void)cancel
{
    ...
    if (self.connection)
    {
        ...
        if (self.isExecuting) self.myexecuting = NO;
        if (!self.isFinished) self.myfinished = YES;
    }
